...
const Alerts = () => {
  ...
  useEffect(() => {
    fetchAlerts();
    // Auto-refresh every 30 seconds for real-time alerts
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'PENDING': return <Notifications color="error" />;
  ...
  const highPriorityAlerts = pendingAlerts.filter(alert => alert.severity === 'HIGH');
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Monitor and manage cyberbullying alerts
          </Typography>
        </Box>
            ...
            <Box>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <Box display="flex" flexDirection="column" gap={1} sx={{ mt: 1 }}>
                  ...
                  <Typography variant="body2">
                    <strong>Confidence:</strong> {(selectedAlert.confidence * 100).toFixed(1)}%
                  </Typography>
                  <Typography variant="body2">
